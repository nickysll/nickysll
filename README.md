# Oi, eu sou a Nicoly 👩🏻‍💻✨
### BI | Data Analytics | Python | SQL | Power BI

# Oi, eu sou a Nicoly Moreira 👩🏻‍💻✨

Sou formada em **Ciências Econômicas** e atualmente estudo **Ciência de Dados**.  
Tenho experiência com **BI, análise de dados, Power BI, SQL e Python**, atuando na criação de dashboards, indicadores, automações e análises para apoiar decisões de negócio.

Atualmente estou aprofundando meus estudos em **Machine Learning Engineering**, versionamento com Git/GitHub e boas práticas para projetos de dados.

---

## Sobre mim

- 📊 Trabalho com **Business Intelligence e Data Analytics**
- 🐍 Uso **Python** para análise, tratamento e automação de dados
- 🧠 Tenho interesse em **Machine Learning, Engenharia de Dados e Analytics Engineering**
- 🗄️ Experiência com consultas SQL e bases em ambiente de dados
- 📈 Gosto de transformar dados em indicadores claros, visuais e úteis para o negócio
- 🚀 Construindo meu portfólio com projetos práticos de dados

---

## Tecnologias e ferramentas

<div align="left">

![Python](https://img.shields.io/badge/Python-111827?style=for-the-badge&logo=python)
![SQL](https://img.shields.io/badge/SQL-111827?style=for-the-badge&logo=postgresql)
![Power BI](https://img.shields.io/badge/Power%20BI-111827?style=for-the-badge&logo=powerbi)
![Pandas](https://img.shields.io/badge/Pandas-111827?style=for-the-badge&logo=pandas)
![Git](https://img.shields.io/badge/Git-111827?style=for-the-badge&logo=git)
![GitHub](https://img.shields.io/badge/GitHub-111827?style=for-the-badge&logo=github)
![AWS](https://img.shields.io/badge/AWS-111827?style=for-the-badge&logo=amazonaws)

</div>

---

## Projetos em destaque

### Machine Learning Engineering
Projeto de estudos focado em boas práticas para projetos de Machine Learning, incluindo organização de diretórios, ambiente virtual, versionamento, validação de ambiente e pipeline inicial.

**Tecnologias:** Python, Git, GitHub, ambientes virtuais, organização de projeto.

---

### Gerenciamento de Estoque
Aplicação em Python para controle de produtos, estoque e movimentações, com foco em lógica de programação, manipulação de dados e organização de código.

**Tecnologias:** Python.

---

### App de Vendas
Projeto para registrar, organizar e analisar informações de vendas, simulando um cenário de negócio.

**Tecnologias:** Python, lógica de programação e análise de dados.

---

## Atualmente estudando

- Python para projetos de dados
- SQL para análise e modelagem
- Machine Learning Engineering
- Estatística aplicada
- Boas práticas com Git e GitHub
- Power BI avançado

---

## GitHub Stats

<div align="center">

![Nicoly's GitHub stats](https://github-readme-stats.vercel.app/api?username=nickysll&show_icons=true&theme=tokyonight&hide_border=true)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=nickysll&layout=compact&theme=tokyonight&hide_border=true)

</div>

---

## Contato

[![LinkedIn](https://img.shields.io/badge/LinkedIn-111827?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/nicolydasilvamoreira)



<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=nickysll&hide_title=false&hide_rank=false&show_icons=true&include_all_commits=true&count_private=true&disable_animations=false&theme=radical&locale=en&hide_border=false&order=1&custom_title=Statistics%20-%20Nicoly%20Moreira" height="150" alt="stats graph" /> <br>
  <img src="https://streak-stats.demolab.com?user=nickysll&locale=en&mode=daily&theme=radical&hide_border=false&border_radius=5&order=3" height="150" alt="streak graph" /> <br>
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=nickysll&radius=16&theme=redical&area=true&order=5" height="300" alt="activity-graph graph"  />
</div>
